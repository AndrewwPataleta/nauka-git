...
class ProfileEditAddressesListFragment :
    ...
    fun providePresenter(): ProfileEditAddressesListPresenter =
        getScope().getInstance(ProfileEditAddressesListPresenter::class.java)
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        contentView.done.setOnClickListener { presenter.selectUpdateUserAddresses() }
        contentView.countryBorn.setOnClickListener { presenter.askForOpenEditCountryBord() }
        contentView.countryLive.setOnClickListener { presenter.askForOpenEditCountryLive() }
        contentView.settlementBorn.setOnClickListener { presenter.askForOpenEditSettlementBord() }
        contentView.settlementLive.setOnClickListener { presenter.askForOpenEditSettlementLive() }
        setFragmentResultListener(SELECTED_COUNTRY_RESULT) { _, bundle ->
            bundle.getParcelable<Country>(SELECTED_COUNTRY)?.let { country ->
                presenter.setSelectedCountry(
                    country = country,
                    countryType = bundle.getParcelable(UPDATE_COUNTRY_TYPE) ?: CountryType.BORN
                )
            }
        }
        setFragmentResultListener(SELECTED_CITY_RESULT) { _, bundle ->
            bundle.getString(SELECTED_CITY_RESULT)?.let { city ->
                presenter.setSelectedCity(
                    city = city,
                    cityType = bundle.getParcelable(UPDATE_SETTLEMENT_TYPE) ?: SettlementType.BORN
                )
            }
        }
    }
    ...
    override fun openSelectCountryForBorn(country: Country) {
        val bundle = Bundle()
        bundle.putString(SELECTED_COUNTRY_ID, country.id)
    ...
    override fun showSuccessToast() {
        Toast.makeText(requireContext(), getString(R.string.data_success_updated), Toast.LENGTH_LONG).show()
    }
}
