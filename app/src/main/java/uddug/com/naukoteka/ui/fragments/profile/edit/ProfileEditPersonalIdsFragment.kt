...
class ProfileEditPersonalIdsFragment : BaseFragment(R.layout.fragment_profile_edit_personal_ids),
    ...
    fun providePresenter(): ProfileEditPersonalIdsPresenter =
        getScope().getInstance(ProfileEditPersonalIdsPresenter::class.java)
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        contentView.done.setOnClickListener { presenter.selectUpdateUserIds() }
        contentView.spinCodeValue.addTextChangedListener { presenter.setCurrentSpinCode(it.toString()) }
        contentView.orchidCodeValue.addTextChangedListener { presenter.setCurrentOrchid(it.toString()) }
        contentView.researcerCodeValue.addTextChangedListener { presenter.setCurrentReserchId(it.toString()) }
    }
    ...
    override fun setSpinCode(spinCode: String) {
        contentView.spinCodeValue.setText(nonAlphanumericRegex.replace(spinCode, ""))
    }

    override fun setOrchid(orchid: String) {
        contentView.orchidCodeValue.setText(nonAlphanumericRegex.replace(orchid, ""))
    }

    override fun setReserch(reserched: String) {
        contentView.researcerCodeValue.setText(nonAlphanumericRegex.replace(reserched, ""))
    }

    override fun userIdsSuccessUpdated() {}

    override fun showIdsUpdatedSuccess() {
        Toast.makeText(requireActivity(), getString(R.string.data_updated_success), Toast.LENGTH_LONG).show()
    }
}
