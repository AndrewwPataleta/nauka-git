...
class ProfileEditContactsFragment : BaseFragment(R.layout.fragment_profile_edit_contacts),
    ...
    fun providePresenter(): ProfileEditContactsPresenter =
        getScope().getInstance(ProfileEditContactsPresenter::class.java)
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        contentView.addNewEmail.setOnClickListener { presenter.askToAddNewEmail() }
        contentView.addNewPhone.setOnClickListener { presenter.askToAddNewPhone() }
        contentView.addNewSite.setOnClickListener { presenter.askToAddNewSite() }
        contentView.done.setOnClickListener { presenter.askToSaveContacts() }
    }
    ...
    override fun setAdditionalSites(sites: List<ContactData>) {
        ...
        sites.forEach { site ->
            AdditionalSiteEditView(requireContext()).attachAdditionalSite(
                contentView.additionalSiteContainer, site
            )
        }
    }
    ...
    override fun setAdditionalPhones(phones: List<ContactData>) {
        ...
        phones.forEach { phone ->
            AdditionalPhoneEditView(requireContext()).attachAdditionalPhone(
                contentView.additionalPhoneContainer, phone
            )
        }
    }
    ...
    override fun setAdditionalEmails(email: List<ContactData>) {
        ...
        email.forEach { item ->
            AdditionalEmailEditView(requireContext()).attachAdditionalEmail(
                contentView.additionalEmailContainer, item
            )
        }
    }
    ...
    override fun addNewEmail(contactDatum: ContactData) {
        AdditionalEmailEditView(requireContext()).attachAdditionalEmail(
            contentView.additionalEmailContainer, contactDatum
        )
    }

    override fun addNewSite(contactDatum: ContactData) {
        AdditionalSiteEditView(requireContext()).attachAdditionalSite(
            contentView.additionalSiteContainer, contactDatum
        )
    }

    override fun addNewPhone(contactDatum: ContactData) {
        AdditionalPhoneEditView(requireContext()).attachAdditionalPhone(
            contentView.additionalPhoneContainer, contactDatum
        )
    }

    override fun showDataUpdated() {
        Toast.makeText(requireActivity(), getString(R.string.contact_data_updated_succesfull), Toast.LENGTH_LONG).show()
    }
}
