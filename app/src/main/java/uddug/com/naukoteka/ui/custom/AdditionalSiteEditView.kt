...
import uddug.com.naukoteka.databinding.ViewProfileAdditionalSiteBinding
...
class AdditionalSiteEditView @JvmOverloads constructor(
    context: Context,
    attrs: AttributeSet? = null,
    defStyleAttr: Int = 0
) : FrameLayout(context, attrs, defStyleAttr) {

    private lateinit var binding: ViewProfileAdditionalSiteBinding

    fun attachAdditionalSite(
        parent: ViewGroup,
        site: ContactData,

        ) {
        binding = ViewProfileAdditionalSiteBinding.inflate(
            LayoutInflater.from(context),
            parent,
            true
        )
        binding.value.setText(site.contact)
