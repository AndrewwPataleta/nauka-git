    ...
    inner class ViewHolder(@LayoutRes layoutRes: Int, parent: ViewGroup) :
        ...
        override fun updateView(item: LaborActivities) {
            ...
            rootView.subTitle.text = item.activityAreasMap.values.joinToString(
                separator = rootView.root.context.getString(R.string.comma)
            )
            rootView.subTitleDirection.text = item.cityAsString.toString()
                .plus(if (item.startWork != null) " • ${LocalDate.parse(item.startWork, dateFormat).year}" else "")
                .plus(if (item.endWork != null) " – ${LocalDate.parse(item.endWork, dateFormat).year} " else "")
        }
    }
}
