...
6. **Выход из звонка**
   1. **Правильный способ**
      - **WCS:** вызвать `Room.leave()` для текущей комнаты.
      - **Бэк:**
        - получает колбэк от WCS, находит все сессии участника и завершает их;
        - изменяет статус участника на **Участвовал (2)**;
        - выполняет постобработку администраторов и зависших звонков.
   2. **Допустимый способ**
      - **WCS:** прекратить публикацию медиа-потока `Room.unpublish()`.
      - **Бэк:**
        - очищает данные о сессии;
        - если больше сессий участника нет, то изменяет статус участника на **Участвовал (2)**;
        - выполняет постобработку администраторов и зависших звонков.

7. **Завершение звонка** (администратор/организатор или любой участник звонка 1×1)
   - REST: `POST /calls/:id/stop`.
   - Бэк:
     - находит все WCS-сессии участников и принудительно завершает их;
     - меняет статус участников на **Участвовал в звонке (2)** или **Пропустил звонок (8)**;
     - рассылает сообщения: всем — `cType = 6` (CALL_ENDED), пропустившим — `cType = 6004` (CALL_END_MISSED);
     - останавливает запись звонка при наличии.
